import {
    ...
    sendAndConfirmTransaction, PublicKey, Signer,
} from "@solana/web3.js";
import {DEFAULT_TIMEOUT} from "./constants";
import {getUnixTs, sleep} from "./helpers";
import {AnchorWallet} from "@solana/wallet-adapter-react";
import {BlockhashAndFeeCalculator, Txn} from "./types";
import {signAndSendTransactionInstructions} from "@solana/spl-name-service";
...
export async function sendTransactionWithRetryWithKeypair(
    ...
    // let signerArr: Signer[] = [];
    signers.forEach(signer => transaction.partialSign(signer));
    // signers.forEach(signer => signerArr.push(signer));

    // let receipt = await sendAndConfirmTransaction(connection, transaction, signers);
    // console.log(receipt);
    // return;
